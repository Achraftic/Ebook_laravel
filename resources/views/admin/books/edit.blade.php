...
@section('content')
    ...
    <section class="">
        ...
        <form action=" {{ route('books.update', $data->id) }} " method="post" enctype="multipart/form-data"
            class=" bookscreate  p-8  space-y-4  gap-4   rounded-xl mx-auto   ">
            @csrf
            ...
            <div class=" sm:grid  sm:grid-cols-2  grid-cols-1 gap-4  sm:space-y-0  space-y-5 ">
                <div>
                    ...
                    <input id="Name" class="form-input w-full" value="{{ $data->name }}" name="name"
                        type="text" placeholder="Example: Book" />
                ...
                <div>
                    ...
                    <input id="author" class="form-input w-full" value="{{ $data->author }}" name="author"
                ...
                <div>
                    ...
                    <select name="category" id="" class="form-input w-full border-gray-300 rounded-md">
                        ...
                        @foreach ($category as $item)
                            <option value="{{ $item->id }}" {{ $data->categoryId == $item->id ? 'selected' : '' }}
                                class="text-slate-500 p-2"> {{ $item->name }} </option>
                        @endforeach
                ...
                <div>
                    ...
                    <input id="rate" class="form-input w-full" name="rate" value="{{ $data->rate }}"
                        max="5" type="number" placeholder="rate" />
                </div>
                ...
                <div class="col-span-2   w-full">
                    ...
                    <textarea name="description" id="description" cols="30" rows="10" class=" w-full">{{ $data->description }}
                    </textarea>
                </div>
            ...
            <div class="flex justify-center items-center flex-col">
                ...
                <label
                    ...
                    <input type="file" id="image" value="{{ $data->image }}" name="image" class="hidden ">
                    ...
                    <label for="value" id='valueImage' class="text-orange-600 "> {{ $data->image }} </label>
                </label>
                <label
                    class="my-4  flex space-x-2 items-center w-full   cursor-pointer p-3 bg-white border-2  hover:border-orange-500 rounded-md font-semibold text-xs text-gray-700   shadow-sm hover:text-gray-500 focus:outline-none  transition ease-in-out duration-150">
                    <input type="file" id="pdf" value="{{ $data->pdf }}" name="pdf" class="hidden ">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div>
                        ...
                        <label for="value" id='valuePdf' class="text-orange-600 ">{{ $data->pdf }} </label>
                    </div>
                </label>
            </div>
            <button
                class="btn rounded-md bg-orange-500 hover:bg-orange-600 text-white sm:w-[150px] w-full p-2 ">Submit</button>
