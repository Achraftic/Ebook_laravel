...
@section('content')
    <section class=" sm:p-5 p-2 gap-3 mt-8 m-auto min-h-max lg:flex   ">
        ...
        <div class="  min-h-[50vh]  md:w-full lg:w-[80%] m-auto p-10 lg:items-start md:items-center md:flex  flex-cols ">
            ...
            <div class=" space-y-2 p-5 md:w-[70%] w-full ">
                ...
                <div class="text-white  flex  space-x-3  mt-10 ">
                    <a href={{ route('pdf.read',$data->pdf )}}
                    ...
                    <a  href={{route('books.downloadPdf',$data->pdf)}} type="submit"
        ...
        <div class=" md:w-[30%] {{(count($BookSame))?" shadow-md":""}}  p-4 mt-4  h-full ">
            @if(count($BookSame))
            ...
            <div class=" max-w-[1000px]  ">
                ...
                <div class="flex justify-center  md:justify-start md:space-x-0 space-x-4 flex-wrap items-center">
                    @foreach ($BookSame as $i)
                        <div class="my-4 p-2 md:basis-full rounded-sm flex bg-slate-100 items-center space-x-3">
                            <div class="md:w-max     ">
                            ...
                            <div class="ml-0">
                                <h1 class="text-mg text-slate-900"> {{ $i->name }} </h1>
                                <p class="text-sm text-slate-400">{{ $i->author }} </p>
                            </div>

                            <a href="{{ route('books.show', $i->id) }}"
                                class="p-2 px-4 bg-orange-500 rounded-sm  text-white ">view</a>
    ...
    <script type="module" defer>
        $(document).ready(function() {
            const booksId = parseInt(window.location.href.substr(window.location.href.lastIndexOf('/') + 1));

            function fetchAndRenderComments() {
                const csrfToken = $('meta[name="csrf-token"]').attr('content');

                $.ajax({
                    url: `http://127.0.0.1:8000/books/book/comment/${booksId}`,
                    method: 'GET',
                    data: {
                        books_id: booksId
                    },
            ...
            function addComment() {
                ...
                const csrfToken = $('meta[name="csrf-token"]').attr('content');
                 console.log('jidj');
                $.ajax({
                    url: `http://127.0.0.1:8000/books/book/comment/add/${booksId}`,
                    method: 'GET',
                    data: {
                        books_id: booksId,
                        comment: comment
            ...
            function renderComments(comments) {
                const html = comments.map(comment => {
                    const imagePath = comment.user.image ? `/imgUser/${comment.user.image}` : '/imgUser/noImg.png';
                    return `
                        <div class="border min-h-[10vh] mt-5 lg:w-[70%] w-full relative border-gray-300 rounded-md text-slate-900">
                            <div class="p-7">
                                <p>${comment.comment}</p>
                                <button type="submit" class="cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute bottom-2 right-4 w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                                    </svg>
                                </button>
                            </div>
                            <div class="absolute -top-6 -left-5 p-2 w-12 rounded-full h-12 overflow-hidden">

                                <img src="${imagePath}" alt="" class=" rounded-full w-full h-full object-cover  ">
                                </div>
                        </div>
                    `;
                }).join('');
